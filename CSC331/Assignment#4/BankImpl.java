     ...
     // first check if there are sufficient resources available
      ...
      for (int i = 0; i < n; i++) {
         ...
         if(!canFinish[i]){
            ...
            for (int j = 0; j < m; j++) {
            //if this thread has not finished (determined from canFinish[i]
            ...
            //							add its allocation to available (make the allocated resources to it available to others)
               ...
               if(need[i][j] > avail[j]){//checks if there is any resource that cannot be accomodated
                  work = false;       
            ...
            if(work){                  //Will execute if needs can be accomodated
               canFinish[i] = true;
            
               for(int j=0;j<m;j++){
                  avail[j] += allocation[i][j]; //add its allocation to available
               }
            }	
      ...
      for(int i=0;i<canFinish.length;i++){
         returnValue = returnValue & canFinish[i]; //Check bitwise equality. Returns bit value
      }
